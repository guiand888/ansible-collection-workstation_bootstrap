...
* `flatpaks_packages_to_install` (list of dictionaries)
  ...
  * `method` (string, *optional*): Specifies how the Flatpak should be installed. Accepted values are `user` (for the current user) or `system` (system-wide). If not specified for an item, it defaults to `flatpaks_install_method_default`.
...
* `flatpaks_install_method_default` (string)
  The default installation method for Flatpaks, applied when `method` is not specified for an individual item in `flatpaks_packages_to_install`.
  * Default: `"user"`

## Example Playbook

To use this role, define the `flatpaks_packages_to_install` variable in your playbook or inventory.
...
- name: Install Flatpak applications
  ...
  vars:
    flatpaks_install_method_default: "user" # Explicitly set default for this playbook
